...
export const createBaseServiceConfig = (config: Config): webpack.Configuration => {
  return {
    ...
    externals: [nodeExternals({
      allowlist: [/util\/.*/],	// https://github.com/liady/webpack-node-externals/issues/8
    })],
    target: "node",
    devtool: "source-map",
    output: {
      path: path.resolve(config.dir, "dist"), // required so that clean-webpack-plugin knows which folder to clean
      filename: "index.js",
